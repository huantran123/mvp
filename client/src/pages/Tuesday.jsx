...
class Tuesday extends React.Component {
  constructor(props) {
    ...
    this.url = 'http://localhost:2000/workout-day/tuesday';
    ...
    $.get('http://localhost:2000/exercises')
      .done((exercises) => {
        ...
        for (var exercise of exercises) {
          if (this.state.tuesdayExercises[exercise._id] === undefined) {
  ...
  getWorkoutDayExercises () {
    $.get(this.url)
      .done((exercises) => {
        ...
        for (var exercise of exercises) {
          tuesdayExercises[exercise._id] = exercise;
        }
        this.setState({
          tuesdayExercises
        });
  ...
  addExercisesToDay (exerciseList) {
    console.log('Exercise is adding to Tuesday list');
    $.post(this.url, {exerciseList: exerciseList})
      .done((exercises) => {
        console.log('Exercise is successfully added to Tuesday list!');
        const tuesdayExercises = {};
        for (var exercise of exercises) {
          tuesdayExercises[exercise._id] = exercise;
        }
  ...
  render() {
    return (
      <div className='body-container'>
        ...
        <div className='cards'>
          {Object.values(this.state.tuesdayExercises).length === 0 ? <div style={{color:'#7b878e'}}>No exercise yet!</div> : null}
          {Object.values(this.state.tuesdayExercises).map((exercise) => (
            <Card
              key={exercise._id}
              _id={exercise._id}
              name={exercise.name}
