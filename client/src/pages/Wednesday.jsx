...
class Wednesday extends React.Component {
  constructor(props) {
    ...
    this.url = 'http://localhost:2000/workout-day/wednesday';
    ...
    $.get('http://localhost:2000/exercises')
      .done((exercises) => {
        ...
        for (var exercise of exercises) {
          if (this.state.wednesdayExercises[exercise._id] === undefined) {
  ...
  getWorkoutDayExercises () {
    $.get(this.url)
      .done((exercises) => {
        ...
        for (var exercise of exercises) {
          wednesdayExercises[exercise._id] = exercise;
        }
        this.setState({
          wednesdayExercises
        });
  ...
  addExercisesToDay (exerciseList) {
    console.log('Exercise is adding to Wednesday list');
    $.post(this.url, {exerciseList: exerciseList})
      .done((exercises) => {
        console.log('Exercise is successfully added to Wednesday list!');
        const wednesdayExercises = {};
        for (var exercise of exercises) {
          wednesdayExercises[exercise._id] = exercise;
        }
  ...
  render() {
    return (
      <div className='body-container'>
        ...
        <div className='cards'>
          {Object.values(this.state.wednesdayExercises).length === 0 ? <div style={{color:'#7b878e'}}>No exercise yet!</div> : null}
          {Object.values(this.state.wednesdayExercises).map((exercise) => (
            <Card
              key={exercise._id}
              _id={exercise._id}
              name={exercise.name}
