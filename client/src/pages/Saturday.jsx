...
class Saturday extends React.Component {
  constructor(props) {
    ...
    this.url = 'http://localhost:2000/workout-day/saturday';
    ...
    $.get('http://localhost:2000/exercises')
      .done((exercises) => {
        ...
        for (var exercise of exercises) {
          if (this.state.saturdayExercises[exercise._id] === undefined) {
  ...
  getWorkoutDayExercises () {
    $.get(this.url)
      .done((exercises) => {
        ...
        for (var exercise of exercises) {
          saturdayExercises[exercise._id] = exercise;
        }
        this.setState({
          saturdayExercises
        });
  ...
  addExercisesToDay (exerciseList) {
    console.log('Exercise is adding to Saturday list');
    $.post(this.url, {exerciseList: exerciseList})
      .done((exercises) => {
        console.log('Exercise is successfully added to Saturday list!');
        const saturdayExercises = {};
        for (var exercise of exercises) {
          saturdayExercises[exercise._id] = exercise;
        }
  ...
  render() {
    return (
      <div className='body-container'>
        ...
        <div className='cards'>
          {Object.values(this.state.saturdayExercises).length === 0 ? <div style={{color:'#7b878e'}}>No exercise yet!</div> : null}
          {Object.values(this.state.saturdayExercises).map((exercise) => (
            <Card
              key={exercise._id}
              _id={exercise._id}
              name={exercise.name}
